...
class HumanoidVanilla(LeggedRobot):
    ...
    def _reset_system(self, env_ids):
        ...
        self.current_step[env_ids] = torch.tensor([[0., 0.155, 0.], [0., -0.155, 0.]], device=self.device, requires_grad=False) # current_step initializatoin
    ...
    def check_termination(self):
        ...
        self.terminated |= torch.any(self.base_pos[:, 2:3] < 0.5, dim=1)
...
# ########################## REWARDS ######################## #
    ...
    def _reward_joint_regularization(self):
        ...
        error += self._negsqrd_exp(
            (self.dof_pos[:, 2]) / self.scales['dof_pos'])
        error += self._negsqrd_exp(
            (self.dof_pos[:, 8]) / self.scales['dof_pos'])
        ...
        error += self._negsqrd_exp((self.dof_pos[:, 7]) / self.scales['dof_pos'])
